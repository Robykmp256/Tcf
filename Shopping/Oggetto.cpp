...
Oggetto::Oggetto( string titolo, int s) {//s è la risposta, titolo è il file txt a cui fa riferimento
    ...
    if (file.is_open()) {
        while (file) {
            getline(file, riga);
            if (j >= (s - 1) * 6 && j <= (s - 1) * 6 +
                                         5) {   //incrementa la i solo nel momento che mi trovo in corrispondenza del prodotto selezionato
                i++;
